...
df = pd.read_stata(
    "/Users/gabrielderome/Desktop/tp/econometrie/TP2/data_angrist_evans.dta"
)
...
ols_model = sm.OLS.from_formula(
    "labforce ~ wkswork2 + hrswork2 + inctot + ftotinc", data=female
)
...
model1_2sls = IV2SLS(
    female[dependent_var], female[regressed_var_list], female[instrument1]
).fit()
# print the summary of the model
...
model2_2sls = IV2SLS(
    female[dependent_var], female[regressed_var_list], female[instrument2]
).fit()
